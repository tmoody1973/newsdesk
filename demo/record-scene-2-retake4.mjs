...
const TRIMS = [
  [0, "Brisha Borden was rated high risk after taking a child's bicycle and scooter. She has not been charged with any new crimes."],
  [1, "ProPublica analyzed risk scores for more than 7,000 people arrested in Broward County, Florida."],
  [2, "Black defendants were 77 percent more likely to be labeled higher risk for future violent crime."],
  [3, "The algorithm was nearly twice as likely to falsely flag black defendants as future criminals."],
  [4, "In 2014, Attorney General Eric Holder warned that risk scores might be injecting bias into the courts."],
];
...
mark("S2 end");
await p.close();
fs.renameSync(await p.video().path(), `${OUT}/s2-wizard-retake4.webm`);

await ctx.close();
await browser.close();
console.log("DONE scene 2 retake4");
